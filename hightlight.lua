...
spacing = 20        -- Espaçamento entre os browsers (em pixels)
...
function reorganize_browsers()
    ...
    -- Filtra os browsers ativos (visíveis) e inativos
    for _, scene_item in ipairs(scene_items) do
        ...
        if string.match(source_name, "^CAM_") then
            if obs.obs_sceneitem_visible(scene_item) then
                ...
                last_browser = scene_item -- Atualiza a última fonte encontrada
            else
    ...
    -- Remove o último browser da lista de ativos
    if last_browser then
        for i, browser in ipairs(active_browsers) do
            if browser == last_browser then
                table.remove(active_browsers, i)
                break
            end
        end
    end
    ...
    handle_browsers(active_browsers, inactive_browsers, last_browser)

    obs.sceneitem_list_release(scene_items)
    obs.obs_scene_release(scene)
end

function handle_browsers(active_browsers, inactive_browsers, last_browser)
    local total_browsers = #active_browsers
    if total_browsers and not last_browser == 0 then return end

    local screen_width = 1920
    local screen_height = 1080

    -- Calcular o espaço vertical disponível para as browsers (com margens superior e inferior)
    local vertical_margin = spacing
    local available_height = screen_height - (vertical_margin * 2) - (total_browsers - 1) * spacing
    local browser_height = available_height / (total_browsers)
    ...
    -- Se houver uma última fonte de browser, posicioná-la no espaço restante
    if last_browser then
        local last_browser_width, last_browser_height
        if total_browsers == 0 then
	    last_browser_width = screen_width * (5/6)
            last_browser_height = last_browser_width * (9/16)
        else
            last_browser_width = screen_width - (3 * spacing + browser_width)
            last_browser_height = last_browser_width * (9/16)
        end

        local scale_x = last_browser_width / screen_width
        local scale_y = last_browser_height / screen_height

        -- Posição da última fonte
        local x_pos, y_pos
        if total_browsers == 0 then
            x_pos = (screen_width - last_browser_width) / 2
            y_pos = (screen_height - last_browser_height) / 2
        else
            x_pos = 2 * spacing + browser_width
            y_pos = (screen_height - last_browser_height) / 2
        end

        -- Configura a posição e escala da última fonte de browser
        show_browser(last_browser, x_pos, y_pos, scale_x, scale_y)
    end
end
...
function hide_browser(scene_item)
    if not scene_item then return end
    if not script_active == true then return end
